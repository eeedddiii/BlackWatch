...
import beanie
from models.mongo_model import db_Challenge, db_Sessions, db_Credentials, db_Leaked, db_Vulnerability, db_Requests
...
file_handler = RotatingFileHandler(os.path.join(log_dir, 'app.log'), maxBytes=1000000, backupCount=1)
...
async def lifespan(app: FastAPI):
    ...
    await beanie.init_beanie(
        database=mongodb.db,
        document_models=[db_Challenge, db_Sessions, db_Credentials, db_Leaked, db_Vulnerability, db_Requests]
    )
...
async def request_middleware(request: Request, call_next):
    ...
    try:
        ...
        if request.url.path not in open_paths:
            ...
            if not session_id:
                logger.warning(f'Access denied for {request.url.path}: Missing session id header')
                return JSONResponse(status_code=401, content={"message": "Missing session id"})
            session = await db_Sessions.get(session_id)
            if not session:
                logger.warning(f'Access denied for {request.url.path}: Invalid or expired session ID')
